...
import { html, md } from "@dral/javascript-basic-serialize/html.js";
...
let throttled_postmessage = throttle((message) => process.send!(message));
...
process.on("message", async (message: CircuitMessage) => {
  if (message.type === "update-notebook") {
    ...
    try {
      await engine.update(notebook);
    } catch (error) {
      // TODO Communicate this back to the user
      console.log(pc.red(pc.bold(`COULDN'T UPDATE ENGINE:`)));
      console.log(pc.red(error.stack));
    }
  }
});
