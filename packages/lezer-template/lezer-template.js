 ...
 * @returns {SyntaxNode}
 */
export let julia_to_ast = (javascript_code) => {
  return /** @type {any} */ (
    javascriptLanguage.parser.parse(javascript_code).topNode.firstChild
  );
};
...
let memo = (
  fn,
  cachekey_resolver = /** @type {any} */ ((x) => x),
  cache = new Map()
) => {
  return /** @type {any} */ (
    (/** @type {P} */ ...args) => {
...
let weak_memo = (fn, cachekey_resolver = (...x) => x) =>
  memo(fn, cachekey_resolver, new ManyKeysWeakMap());
...
/**
 * @typedef TreeCursor
 * @type {import("@lezer/common").TreeCursor}
 *
 * @typedef SyntaxNode
...
export let match_template = (
  ...
  if (verbose) {
    console.group(
      "Current haystack:",
      !haystack_cursor ? null : haystack_cursor.node.name
    );
    ...
    try {
      ...
      console.log(
        `haystack_cursor:`,
        !haystack_cursor ? null : haystack_cursor.node.toString()
      );
    ...
    if ("pattern" in template) {
      ...
      if (verbose) {
        ...
        console.groupCollapsed(
          `Matching against pattern: ${template.pattern.name}()`
        );
      }
      ...
      if (verbose) {
        if (did_match) {
          ...
          if (!isEqual(matches, matches_before_matching)) {
            let new_matches = omit(
              matches,
              Object.keys(matches_before_matching)
            );
            // prettier-ignore
      ...
      if (!haystack_cursor) {
        if (node.name === "end") {
          verbose &&
            console.log(`✅ No node left to match, but it was the end anyway`);
          return true;
        }
      ...
      if (haystack_cursor.name !== node.name) {
        verbose &&
          console.log(
            `❌ because name mismatch "${haystack_cursor.name}" !== "${node.name}"`
          );
        return false;
      }
      ...
      if (haystack_cursor.firstChild()) {
        try {
          ...
          for (let template_child of children) {
            if (is_last_from_haystack) {
              verbose &&
                console.log(
                  `Haystack is empty, but there are more children in template... lets see`
                );
              let child_does_match = match_template(
                null,
                template_child,
                matches,
                verbose
              ...
              if (!child_does_match) {
                verbose &&
                  console.log(
                    `❌ template child did not accept null for an answer`,
                    template_child,
                    haystack_cursor.toString()
                  );
                return false;
              }
              verbose &&
                console.log(`👌🏽 This template child was fine with null`);
              continue;
            }
            ...
            while (
              haystack_cursor.name === "Comment" ||
              haystack_cursor.name === "BlockComment"
            ) {
        ...
        if (template.children.length !== 0) {
          verbose &&
            console.log(
              `Haystack node is empty, but template has children... lets see`
            );
          ...
          for (let child of template.children) {
            if (!match_template(null, child, matches, verbose)) {
              verbose &&
                console.log(
                  `❌ because child template wasn't okay with having no children`,
                  child
                );
              return false;
            }
          ...
          verbose &&
            console.log(
              `✅ All template children we're fine with having no children to check on`
            );
          return true;
        } else {
...
export class JuliaCodeObject {
  ...
  constructor(template, substitutions) {
    ...
    for (let [string_part, substitution] of zip(
      template.slice(1),
      substitutions
    )) {
      if (substitution instanceof JuliaCodeObject) {
        flattened_template[flattened_template.length - 1] +=
          substitution.template[0];
        for (let [sub_string_part, sub_substitution] of zip(
          substitution.template.slice(1),
...
let substitutions_to_template = (ast, substitutions) => {
  ...
  return {
    ...
    from: ast.from,
    to: ast.to,
    children: Array.from(child_nodes(ast)).map((node) =>
      substitutions_to_template(node, substitutions)
    ),
    node: ast,
  };
};
...
export let to_template = function* (julia_code_object) {
  TEMPLATE_CREATION_VERBOSE &&
    console.group(
      `to_template(`,
      typeof julia_code_object === "function"
        ? julia_code_object.name + "()"
        : julia_code_object,
      `)`
    );
  try {
    if (julia_code_object instanceof JuliaCodeObject) {
      ...
      let subsitions = [];
      for (let [string_part, substitution] of zip(
        julia_code_object.template,
        julia_code_object.substitutions
      )) {
        ...
        if (substitution) {
          ...
          let substitution_code = intermediate_value(
            substitution_generator.next()
          );
      ...
      if (TEMPLATE_CREATION_VERBOSE) {
        ...
        console.log(
          `template_node:`,
          node_to_explorable(template_node.node.cursor())
        );
        console.log(`subsitions:`, subsitions);
        console.log(
          `substitution_with_proper_position:`,
          substitution_with_proper_position
        );
      }

      let result = substitutions_to_template(
        template_node.node,
        substitution_with_proper_position
      );
 ...
 * @param {any[]} substitutions
 * */
export let js_dynamic = weak_memo((template, ...substitutions) => {
...
export let template = weak_memo1((julia_code_object) => {
  ...
  return /** @type {Matcher} */ ({
    ...
    match(haystack_cursor, verbose = false) {
      ...
      if (
        "node" in template_description &&
        template_description.node.name !== haystack_cursor.name
      )
        return;
      ...
      verbose &&
        console.groupCollapsed(`Starting a match at ${haystack_cursor.name}`);
      try {
...
export let as_string = weak_memo1(
  (/** @type {JuliaCodeObject} */ julia_code_object) => {
    let template_generator = to_template(julia_code_object);
    let julia_to_parse = intermediate_value(template_generator.next());
    // @ts-ignore
...
export let as_node = weak_memo1(
  (/** @type {JuliaCodeObject} */ julia_code_object) => {
    return julia_to_ast(as_string(julia_code_object));
  }
);

export let as_doc = weak_memo1(
  (/** @type {JuliaCodeObject} */ julia_code_object) => {
...
function* narrow_template(template_generator) {
  ...
  if (
    ast.node.firstChild &&
    ast.node.from === ast.node.firstChild.from &&
    ast.node.to === ast.node.firstChild.to
  ) {
...
export const t = /** @type {const} */ ({
  ...
  something_with_the_same_type_as: weak_memo1((what) => {
    return function* something_with_the_same_type_as() {
      ...
      return {
        pattern: function something_with_the_same_type_as(
          haystack,
          matches,
          verbose = false
        ) {
  ...
  VariableName: function* VariableName() {
    yield "VariableName";
    return {
      pattern: function VariableName(haystack, matches, verbose = false) {
        return haystack != null && narrow_name(haystack) === "VariableName";
      },
    };
  },
  /** @type {Substitution} */
  ...
  String: function* String() {
    ...
    return {
      pattern: function String(haystack, matches, verbose = false) {
        return haystack != null && narrow_name(haystack) === "String";
      },
    };
  },
  /** @type {Substitution} */
  Property: function* Property() {
    yield `"A113"`;
    return {
      pattern: function Property(haystack, matches, verbose = false) {
        return haystack != null && narrow_name(haystack) === "Property";
      },
    };
  },
});
 ...
 */
export let take_little_piece_of_template = weak_memo(
  (template, meta_template) => {
    ...
    if ((match = meta_template.match(template_ast))) {
      ...
      while (
        content.firstChild &&
        content.firstChild.from == content.from &&
        content.firstChild.to == content.to
      ) {
      ...
      return /** @type {Matcher} */ ({
        ...
        match(haystack_cursor, verbose = false) {
          if (haystack_cursor.type.isError) {
            verbose &&
              console.log(
                `❌ Short circuiting because haystack(${haystack_cursor.name}) is an error`
              );
            return false;
          }
          if ("cursor" in haystack_cursor)
          ...
          for (let possible_parent of possible_parents) {
            if (haystack_cursor.type === possible_parent) {
              ...
              if (haystack_cursor.firstChild()) {
                if (
                  haystack_cursor.from === parent_from &&
                  haystack_cursor.to === parent_to
                ) {
                  verbose &&
                    console.log(
                      `✅ Matched parent, going one level deeper (${possible_parent})`
                    );
                  depth++;
                } else {
                  haystack_cursor.parent();
...
export let narrow = (node) => {
  if (
    node.firstChild &&
    node.firstChild.from === node.from &&
    node.firstChild.to === node.to
  ) {
...
export function iterate_with_cursor({
  ...
  while (true) {
    ...
    if (
      cursor.from <= to &&
      cursor.to >= from &&
      (cursor.type.isAnonymous || enter(cursor) !== false)
    ) {
